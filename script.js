...
const codeBuddyData = {
    ...
    "GIT COMMIT": {
        ...
        questions: [
            ...
            { q: "Şu ana kadar atılmış olan tüm commit geçmişini listemek için hangisi yazılır?", options: ["git status", "git log", "git history", "git show"], answer: "git log" },
...
if (registerForm) {
    registerForm.addEventListener('submit', (e) => {
        ...
        // Kullanıcı adının benzersiz olup olmadığını sorgula
        db.collection("users").doc(username).get().then((doc) => {
            if (doc.exists) {
                alert("🚨 Bu kullanıcı adı sistemde zaten kayıtlı!");
            } else {
                // Yoksa Firebase Firestore'a kaydet
                ...
                .then(() => {
                    ...
                    document.getElementById('tabLogin').click(); // Otomatik giriş sekmesine yönlendir
                })
...
if (loginForm) {
    loginForm.addEventListener('submit', (e) => {
        ...
        // Firestore'dan girilen kullanıcı ismini getir
        db.collection("users").doc(username).get().then((doc) => {
            if (doc.exists) {
                const userData = doc.data();
                // Veritabanındaki şifreyle eşleşiyor mu kontrol et
...
// Sonsuz Dinamik Arama Motoru ve Firebase Bulut Bağlantısı (ONARILDI VE TAMAMLANDI)
if (searchBtn) {
    searchBtn.addEventListener('click', () => {
        ...
        if (!codeBuddyData[upperQuery]) {
            codeBuddyData[upperQuery] = {
                ...
                output: `[Sistem Çıktısı: ${query} entegrasyonu stabil.]`,
                questions: [
                    { q: `${query} temel olarak ne amaçla kullanılır?`, options: ["Veri saklama", "İşlem kontrolü", "Arayüz tasarımı", "Genel yapı"], answer: "İşlem kontrolü" }
                ]
            };
            initDashboard();
        }
